...
func (ph *proxyHandler) ServeHTTP(w http.ResponseWriter, requestReceived *http.Request) {
	...
	hostHeader := requestReceived.URL.Host
	if hostHeader == "" {
	...
	fmt.Println("Request host header: ", hostHeader)
	fmt.Println("Host path: ", requestReceived.URL.Path)
	fmt.Println("Request query: ", requestReceived.URL.RawQuery)
...
func main() {
	fmt.Println("Creating a listener on 0.0.0.0:9090")
